...
const Stats = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const highestScoreMatch = matchesData.reduce((highest, current) => {
...
const Stats = () => {
  ...
  const [showHighestMatchDetails, setShowHighestMatchDetails] = useState(false);
  ...
  const handleHighestScorePress = () => {
    if (stats.batting.highestMatchId) {
      setShowHighestMatchDetails(true);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          <Card style={styles.card}>
            <Card.Content>
              ...
              <View style={styles.statsGrid}>
                ...
                <TouchableOpacity 
                  style={styles.statItem}
                  onPress={handleHighestScorePress}
                >
                  <Text style={styles.statLabel}>Highest</Text>
                  <Text style={[styles.statValue, styles.hyperlink]}>
                    {stats.batting.highest}
      ...
      <Portal>
        <Dialog
          visible={showHighestMatchDetails}
          onDismiss={() => setShowHighestMatchDetails(false)}
          style={styles.dialog}
        >
          <Dialog.Title>Highest Score Match</Dialog.Title>
          <Dialog.Content>
            <Text style={styles.matchDetailsText}>
              {stats.batting.highestMatchId && formatMatchDetails(matches.find(m => m.id === stats.batting.highestMatchId)!)}
            </Text>
            <Text style={styles.matchScoreText}>
              {stats.batting.highest} runs
            </Text>
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={() => setShowHighestMatchDetails(false)}>Close</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
    </SafeAreaView>
