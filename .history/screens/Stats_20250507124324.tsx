    ...
    <SafeAreaView style={styles.container}>
      ...
      <Portal>
        <Dialog
          ...
          onDismiss={() => setShowMatchDetails({ type: null, matchId: null })}
          style={styles.dialog}
        >
          <Dialog.Title>
            {showMatchDetails.type === 'highest' && 'Highest Score Match'}
            {showMatchDetails.type === 'fifty' && '50+ Score Match'}
            {showMatchDetails.type === 'hundred' && '100+ Score Match'}
            {showMatchDetails.type === 'notOut' && 'Not Out Match'}
          </Dialog.Title>
          <Dialog.Content>
            {showMatchDetails.matchId && (
              <>
                <Text style={styles.matchDetailsText}>
                  {formatMatchDetails(matches.find(m => m.id === showMatchDetails.matchId)!)}
                </Text>
                <Text style={styles.matchScoreText}>
                  {matches.find(m => m.id === showMatchDetails.matchId)?.batting.runs} runs
                  {showMatchDetails.type === 'notOut' && ' (Not Out)'}
                </Text>
              </>
            )}
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={() => setShowMatchDetails({ type: null, matchId: null })}>Close</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
    </SafeAreaView>
