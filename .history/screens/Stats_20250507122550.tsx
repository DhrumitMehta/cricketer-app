...
const Stats = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      // Calculate fielding stats first
      const fieldingStats = {
      ...
      console.log('Basic fielding stats:', fieldingStats);

      // Calculate contributions and drops per match
      const totalContributions = fieldingStats.infieldCatches + 
                               fieldingStats.boundaryCatches + 
                               fieldingStats.directRunouts + 
                               fieldingStats.indirectRunouts;
      
      const contributionsPerMatch = fieldingStats.matches > 0 ? 
        totalContributions / fieldingStats.matches : 0;

      const dropsPerMatch = fieldingStats.matches > 0 ? 
        fieldingStats.drops / fieldingStats.matches : 0;

      console.log('Calculated per match stats:', {
        totalContributions,
      ...
      console.log('Final fielding stats:', newStats.fielding);
  ...
  console.log('Rendering fielding stats:', stats.fielding);
