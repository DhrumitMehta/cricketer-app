...
const safeToFixed = (value: number | undefined | null, digits: number = 1): string => {
  if (value === undefined || value === null || isNaN(value)) {
    console.log('Invalid value for toFixed:', value);
    return '0.0';
  }
  ...
  const fetchStats = async () => {
    try {
      ...
      if (filtered.length > 0) {
        // Calculate stats only if we have matches
        newStats.batting = {
          matches: filtered.length,
          runs: filtered.reduce((sum, match) => sum + (match.batting.runs || 0), 0),
          balls: filtered.reduce((sum, match) => sum + (match.batting.balls || 0), 0),
          average: filtered.filter(match => !match.batting.not_out).length > 0 ? 
            filtered.reduce((sum, match) => sum + (match.batting.runs || 0), 0) / 
            filtered.filter(match => !match.batting.not_out).length : 0,
          strikeRate: filtered.reduce((sum, match) => sum + (match.batting.balls || 0), 0) > 0 ?
            (filtered.reduce((sum, match) => sum + (match.batting.runs || 0), 0) * 100) /
            filtered.reduce((sum, match) => sum + (match.batting.balls || 0), 0) : 0,
          highest: Math.max(...filtered.map(match => match.batting.runs || 0)),
          highestMatchId: filtered.length > 0 ? 
            filtered.reduce((highest, current) => 
              (current.batting.runs || 0) > (highest.batting.runs || 0) ? current : highest
            ).id : '',
          notOuts: filtered.filter(match => match.batting.not_out).length,
          dots: filtered.reduce((sum, match) => sum + (match.batting.dots || 0), 0),
          singles: filtered.reduce((sum, match) => sum + (match.batting.singles || 0), 0),
          doubles: filtered.reduce((sum, match) => sum + (match.batting.doubles || 0), 0),
          triples: filtered.reduce((sum, match) => sum + (match.batting.triples || 0), 0),
          fours: filtered.reduce((sum, match) => sum + (match.batting.fours || 0), 0),
          sixes: filtered.reduce((sum, match) => sum + (match.batting.sixes || 0), 0),
          dotPercentage: filtered.reduce((sum, match) => sum + (match.batting.balls || 0), 0) > 0 ?
            (filtered.reduce((sum, match) => sum + (match.batting.dots || 0), 0) * 100) /
            filtered.reduce((sum, match) => sum + (match.batting.balls || 0), 0) : 0,
          boundaryPercentage: filtered.reduce((sum, match) => sum + (match.batting.runs || 0), 0) > 0 ?
            ((filtered.reduce((sum, match) => sum + (match.batting.fours || 0), 0) * 4 +
              filtered.reduce((sum, match) => sum + (match.batting.sixes || 0), 0) * 6) * 100) /
            filtered.reduce((sum, match) => sum + (match.batting.runs || 0), 0) : 0,
          ballsPerBoundary: (filtered.reduce((sum, match) => sum + (match.batting.fours || 0), 0) +
            filtered.reduce((sum, match) => sum + (match.batting.sixes || 0), 0)) > 0 ?
            filtered.reduce((sum, match) => sum + (match.batting.balls || 0), 0) /
            (filtered.reduce((sum, match) => sum + (match.batting.fours || 0), 0) +
              filtered.reduce((sum, match) => sum + (match.batting.sixes || 0), 0)) : 0
        };
      }
            ...
            <Card.Content>
              ...
              <View style={styles.statsGrid}>
                ...
                <View style={styles.statItem}>
                  ...
                  <Text style={styles.statValue}>{safeToFixed(stats.batting.strikeRate)}</Text>
                </View>
                <TouchableOpacity 
                ...
                <View style={styles.statItem}>
                  ...
                  <Text style={styles.statValue}>{safeToFixed(stats.batting.dotPercentage)}%</Text>
                </View>
                <View style={styles.statItem}>
                  <Text style={styles.statLabel}>Bdry Run %</Text>
                  <Text style={styles.statValue}>{safeToFixed(stats.batting.boundaryPercentage)}%</Text>
                </View>
                <View style={styles.statItem}>
                  <Text style={styles.statLabel}>Balls/Bdry</Text>
                  <Text style={styles.statValue}>{safeToFixed(stats.batting.ballsPerBoundary)}</Text>
                </View>
                <View style={styles.statItem}>
